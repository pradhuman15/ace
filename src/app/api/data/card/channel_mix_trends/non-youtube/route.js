...
export async function POST(request) {
  ...
  (
      CASE
          ${conversionData.map(conversion => `
              WHEN (cast(floodlight_id as int64) in (${conversion.floodlights.join(",")}))
  ...
  const query = `
    ...
    WITH filtered_data AS (
      ...
      WHERE
          line_item_type = "Real-time bidding (Video)"
  ...
  try {
    ...
    assert(rows.length > 0, "Channel-Mix-Trends-Non-Youtube is empty");
    ...
    console.error(`Error while fetching channel-mix-trends-non-youtube : ${error}`);
    return new Response(
        JSON.stringify({ error: `Error while fetching channel-mix-trends-non-youtube data` }),
        {
            status: 500,
            headers: { "Content-Type" : "application/json" }
        }
